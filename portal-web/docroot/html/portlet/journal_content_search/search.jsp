...
<c:choose>
	<c:when test="<%= windowState.equals(WindowState.MAXIMIZED) %>">
		...
		<aui:form action="<%= searchURL %>" method="post" name="fm">
			...
			<aui:input align="absmiddle" alt='<%= LanguageUtil.get(pageContext, "search") %>' border="0" cssClass="lfr-search-button" inlineField="<%= true %>" label="" name="search" src='<%= HtmlUtil.escapeHREF(themeDisplay.getPathThemeImages()) + "/common/search.png" %>' title="search" type="image" />
